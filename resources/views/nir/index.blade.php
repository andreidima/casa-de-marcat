            ...
            <div class="col-lg-8" id="produse">
                <form class="needs-validation" novalidate method="GET" action="{{ route('nir.index') }}">
                    ...
                    <div class="row input-group custom-search-form justify-content-center align-self-end">
                        ...
                        <a class="btn btn-sm bg-secondary text-white col-md-4 border border-dark rounded-pill" href="{{ route('nir.index') }}" role="button">
        ...
        <div class="card-body px-0 py-3">
            ...
            <div class="table-responsive rounded">
                <table class="table table-striped table-hover table-sm rounded"> 
                    ...
                    <tbody>      
                        @forelse ($produse_intrate->groupby('produs_id') as $produse_grupate_dupa_nume)         
                            <tr>                  
                                <td align="">
                                ...
                                <td>
                                    {{-- <b>{{ $produs->nume ?? '' }}</b> --}}
                                    <b>{{ ($produse_grupate_dupa_nume->first()->nume) }}</b>
                                </td>
                                <td>
                                    buc
                                </td>
                                <td class="text-right">
                                    {{ $produse_grupate_dupa_nume->sum('cantitate') - $produse_grupate_dupa_nume->sum('cantitate_initiala')}}
                                </td>
                                <td class="text-right">
                                    {{ $produse_grupate_dupa_nume->first()->pret_de_achizitie ? round(($produse_grupate_dupa_nume->first()->pret_de_achizitie / 1.19), 2) : '' }}
                                </td>
                                <td class="text-right">
                                    {{ $produse_grupate_dupa_nume->first()->pret_de_achizitie ? 
                                        (
                                            round(($produse_grupate_dupa_nume->first()->pret_de_achizitie / 1.19), 2) 
                                            * 
                                            ($produse_grupate_dupa_nume->sum('cantitate') - $produse_grupate_dupa_nume->sum('cantitate_initiala'))
                                        ) : '' }}
                                </td>
                                <td class="text-right">
                                    {{ $produse_grupate_dupa_nume->first()->pret_de_achizitie ? 
                                        (
                                            round(($produse_grupate_dupa_nume->first()->pret_de_achizitie * 0.19), 2) 
                                            * 
                                            ($produse_grupate_dupa_nume->sum('cantitate') - $produse_grupate_dupa_nume->sum('cantitate_initiala'))
                                        ) : '' }}
                                </td>
                                <td class="text-right">
                                    {{ $produse_grupate_dupa_nume->first()->pret }}
                                </td>
                                <td class="text-right">
                                    {{ $produse_grupate_dupa_nume->first()->pret ? 
                                        (
